...
class research extends Command
{
     ...
     * @var string
     */
    protected $signature = 'research {time?}';
...
//            $time  = date('Y-m-d', strtotime($time . ' +1 day'));
            $datas = app(Racing10::class)->whereRaw("DATE_FORMAT(time, '%Y %m %d') = DATE_FORMAT('" . $time . "', '%Y %m %d')")->get();
...
//        }
    ...
    {
        app(SlackNotify::class)->setMsg("$msg $count 連")->notify();
    }
